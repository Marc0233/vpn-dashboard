#!/bin/bash
# =============================================================================
#  VPN Dashboard — Installer
#  Installs a web-based OpenVPN management dashboard on any Linux server.
#  Supports: Ubuntu/Debian · CentOS/RHEL/Fedora · Arch Linux
# =============================================================================
set -euo pipefail

# ── Colours ───────────────────────────────────────────────────────────────────
GR='\033[0;32m'; BL='\033[0;34m'; YL='\033[1;33m'
RD='\033[0;31m'; CY='\033[0;36m'; BD='\033[1m'; NC='\033[0m'
ok()   { echo -e "${GR}  ✔  ${NC}$*"; }
info() { echo -e "${BL}  →  ${NC}$*"; }
warn() { echo -e "${YL}  ⚠  ${NC}$*"; }
...
cat << 'BANNER'
  ...
  Web-based OpenVPN management dashboard
  https://github.com/vpn-dashboard
...
# ── Uninstall ─────────────────────────────────────────────────────────────────
do_uninstall() {
    echo -e "\n${RD}${BD}Uninstalling VPN Dashboard...${NC}\n"
    systemctl stop  "$SERVICE"  2>/dev/null || true
    systemctl disable "$SERVICE" 2>/dev/null || true
    rm -f "/etc/systemd/system/${SERVICE}.service"
    systemctl daemon-reload
    rm -rf "$INSTALL_DIR"
    ok "VPN Dashboard removed."
    exit 0
}
...
detect_os() {
    ...
    else die "Unsupported package manager. Install python3, pip, openvpn and flask manually."; fi
...
# ── Interface detection ───────────────────────────────────────────────────────
detect_iface() {
    # Try to find the default route interface, then first non-loopback
    DETECTED=$(ip route 2>/dev/null | awk '/^default/{print $5; exit}')
    if [[ -z "$DETECTED" ]]; then
        DETECTED=$(ip -o link show 2>/dev/null | awk -F': ' '!/lo/{print $2; exit}')
    fi
    echo "${DETECTED:-eth0}"
}
...
echo
echo -e "${BD}Summary${NC}"
echo -e "  Interface : ${CY}${LAN_IFACE}${NC}"
echo -e "  Port      : ${CY}${DASH_PORT}${NC}"
echo -e "  User      : ${CY}${DASH_USER}${NC}"
echo
...
mkdir -p "$(dirname "$CONNECT_SH")"
ok "Directories created"

# ── Hash password (Python) ────────────────────────────────────────────────────
PW_HASH=$(python3 -c "
import hashlib
print(hashlib.sha256(f'vpndash2025:${DASH_PASS}'.encode()).hexdigest())
")
...
cat > "${INSTALL_DIR}/config.json" << JSONEOF
{
  "lan_iface":  "${LAN_IFACE}",
  "port":       ${DASH_PORT},
  "connect_sh": "${CONNECT_SH}"
}
JSONEOF
ok "config.json written"

# ── Write dashboard_auth.json ─────────────────────────────────────────────────
cat > "${INSTALL_DIR}/dashboard_auth.json" << AUTHEOF
{"username": "${DASH_USER}", "password_hash": "${PW_HASH}"}
AUTHEOF
ok "Dashboard credentials set"

# ── Create placeholder connect.sh if not present ─────────────────────────────
if [[ ! -f "$CONNECT_SH" ]]; then
    echo '#!/bin/bash' > "$CONNECT_SH"
    echo '# This file is managed by VPN Dashboard — do not edit manually' >> "$CONNECT_SH"
    chmod +x "$CONNECT_SH"
    warn "Created placeholder ${CONNECT_SH} — the dashboard will populate this when you connect."
fi

# ── Write app.py ──────────────────────────────────────────────────────────────
echo -e "\n${BD}Writing app.py...${NC}"
...
ok "app.py written ($(wc -l < ${INSTALL_DIR}/app.py) lines)"

# ── Systemd service ───────────────────────────────────────────────────────────
echo -e "\n${BD}Creating systemd service...${NC}"
...
# ── Firewall: allow dashboard port ────────────────────────────────────────────
if command -v ufw &>/dev/null && ufw status | grep -q "active"; then
    ufw allow "$DASH_PORT/tcp" comment "VPN Dashboard" &>/dev/null
    ok "UFW: opened port $DASH_PORT"
fi
...
# ── Get LAN IP for summary ────────────────────────────────────────────────────
LAN_IP=$(ip addr show "$LAN_IFACE" 2>/dev/null | awk '/inet /{print $2}' | cut -d/ -f1 | head -1)
LAN_IP="${LAN_IP:-<your-server-ip>}"

# ── Done ──────────────────────────────────────────────────────────────────────
echo
echo -e "${GR}${BD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${GR}${BD}  VPN Dashboard installed successfully!${NC}"
echo -e "${GR}${BD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo
echo -e "  ${BD}Dashboard URL${NC}  http://${LAN_IP}:${DASH_PORT}"
echo -e "  ${BD}Username${NC}       ${DASH_USER}"
echo -e "  ${BD}Password${NC}       ${DASH_PASS}"
echo
echo -e "${YL}${BD}  Next steps:${NC}"
echo -e "  1. Open the dashboard and change your password"
echo -e "  2. Click your VPN provider, enter your credentials and upload a .ovpn config file"
echo -e "     (Download .ovpn files from your provider's website — see links in the dashboard)"
echo -e "  3. Hit Connect next to your config file to start the VPN"
echo -e "  4. Enable the kill switch to block traffic if the VPN ever drops"
echo
echo -e "  ${BD}Manage service${NC}   systemctl [start|stop|restart|status] $SERVICE"
echo -e "  ${BD}View logs${NC}         journalctl -u $SERVICE -f"
echo -e "  ${BD}Uninstall${NC}         bash install.sh --uninstall"
echo
